...
public class Complex {
	...
	public Complex division(Complex divisor) {
		...
		if (divisor.a1 == 0.0 && divisor.b1 == 0.0) {
			System.out.println("no se puede dividir por cero1");
			return null;
		} else {
	...
	public double angulo() {
		double ang = (double)Math.round((Math.toDegrees(Math.atan2(b1,a1))) * 100d) / 100d;
		return ang;
	}
	 ...
	 */
	public double fase() {
		return (double)Math.round((Math.toDegrees(Math.atan2(b1,a1))) * 100d) / 100d;
	}
	...
	public int hashCode() {
		...
		hash = 53 * hash + (int) (Double.doubleToLongBits(this.b1) ^ (Double.doubleToLongBits(this.b1) >>> 32));
		return hash;
	}
}